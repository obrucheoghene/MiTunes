...
const ForgotPasswordModal = () => {
  ...
  const onSubmit: SubmitHandler<MagicLinkFormValues> = async (values) => {
    ...
    setMessage(null);
   try {
    const recoverPasswordURL = `${APP_BASE_URL}/recover-password`
    await appwriteWebClientAccount.createRecovery(values.email, recoverPasswordURL);
    ...
    <Modal title="Forgot Password"
      ...
      <form onSubmit={handleSubmit(onSubmit)} className=" flex flex-col gap-y-4">
        ...
        <Button disabled={isLoading} type="submit" className=" rounded-md text-white">
          {isLoading ? 'Sending Instructions' : 'Send Password Reset Instructions'}
        </Button>
